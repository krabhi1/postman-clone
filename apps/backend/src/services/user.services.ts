...
import { generateUserSignInToken, refreshUserSignInToken, verifyUserSignInToken } from "../jwt/user.jwt.js";
import { getGoogleTokens, getUserProfile } from "../others/google.utils.js";
import { ResultCode, makeResultFrom, makeResult, makeErrorResult } from "../others/utils.js";
...
export async function handleGoogleSignInFromCode(code: string) {
    ...
    if (!gResult.data || gResult.data.length == 0) {
        if (!tokensResult.data.refresh_token) {
            return makeResultFrom({
                code: ResultCode.GOOGLE_MISSING_REFRESH_TOKEN, result
            })
        }
...
export async function handleRefreshAccessToken(oldToken: string) {
    ...
    const vResult = refreshUserSignInToken(oldToken);
    if (!vResult.data) {
        return makeResultFrom({
            other: vResult,
            result
    ...
    const token = generateUserSignInToken({ userId: vResult.data.userId });
