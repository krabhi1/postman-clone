...
export default function Home() {
    ...
    const profile = appStore((state) => state.profile);
    const workspaceGroup = appStore((state) => state.workspaceGroup);
    async function init() {
      const userResult = await userApi.getProfile();
      if (userResult.data) {
        appStore.getState().setProfile(userResult.data);
      }
      ...
      if (workspacesResult.data) {
        appStore.getState().setWorkspaces(workspacesResult.data);
      }
    ...
    async function logOut() {
      appStore.getState().setProfile(undefined);
      clearAuthToken();
      await sleep(1000);
    ...
    async function handleCreateWorkspace() {
      ...
      if (result.data) {
        appStore.getState().addWorkspace(result.data);
      }
    }
    ...
    const handleDeleteWorkspace = useCallback(async (id: string) => {
      appStore.getState().deleteWorkspace(id);
